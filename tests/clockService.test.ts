...
import {
  ...
  formatClock, isTimed, TIME_CONTROLS, type ClockState, type TimeControl,
...
describe('clockService — hamle uygulama', () => {
  it('işleyen taraftan geçen süre düşer, increment eklenir', () => {
    ...
    const after = applyMoveToClock(c, CTRL, 'w');
  ...
  it('sırası olmayan tarafın süresi düşmez (savunma)', () => {
    ...
    const after = applyMoveToClock(c, CTRL, 'w'); // beyaz oynamaya çalışıyor ama sıra siyahta
  ...
  it('sınırsız saatte hamle state değiştirmez', () => {
    ...
    expect(applyMoveToClock(c, UNLIMITED, 'w')).toBe(c);
  });
